...
public class PlayerController : MonoBehaviour
{
    ...
    private bool m_BuyShield = false;
    ...
    {
        m_ActiveShield = GameController.Instance.IsShieldActive();
    }
    ...
    private void UpdateMovement()
    {
        ...
        if(m_BuyShield)
        {
            GameController.Instance.HandleShieldUsed();
            m_BuyShield = false;
        }
    ...
    public void Collision(float m_ForceModifier)
    {
        if (!m_ActiveShield)
        {
            Kill();
            Rigidbody.AddForce(Vector3.up * m_ForceModifier);
        }
